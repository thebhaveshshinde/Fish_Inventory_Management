<template>
  <main class="flex flex-col h-screen pt-20 overflow-y-scroll">
    ...
    <div v-if="fishermans && fishermans?.length > 0"
      ...
      <div v-for="fisherman in filteredFishermans" :key="fisherman.id"
        ...
        <span class="font-medium text-teal-600 capitalize">{{ fisherman?.locality }}
        </span>
...
const filteredFishermans = computed(() => {
  return fishermans.value?.filter(
    (fisherman) =>
      fisherman.vendorsid === currentVendor.id &&
      Object.values(fisherman).some((value) =>
        typeof value === 'string' && value.toLowerCase().includes(q.value.toLowerCase())
      )
  ) || [];
})
